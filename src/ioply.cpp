...
std::vector<Eigen::Vector3f> readply(const char * fname) {
    std::vector<Eigen::Vector3f> outv;
    ...
    int n = X.size();
    for (int i=0; i<n; i++) {
        outv.push_back(Eigen::Vector3f(X[i], Y[i], Z[i]));
    }
    return outv;
}

int writetoply(happly::PLYData &ply, std::vector<float> const &channelv, std::string channelname, std::string fname) {
    ...
    ply.getElement("vertex").addProperty<float>(channelname, channelv);
    ply.write(fname);
    return 0;
}
